 ...
 * device is never touched. Root LaunchDaemon (dist/dev.mavergreen.voodooinputmavericks.linkstated.plist)
...
static dispatch_queue_t reconnect_queue(void) {
    ...
    dispatch_once(&once, ^{ q = dispatch_queue_create("dev.mavergreen.mt2.reconnect", DISPATCH_QUEUE_SERIAL); });
    return q;
}
