...
`mt2_linkstated` already use). This sidesteps UHE/blued's plist machinery entirely: host pages the device at boot
...
Keep it in the same family as `tools/mt2_linkstated` (already a RunAtLoad IOBluetooth daemon).
...
`tools/mt2_linkstated.m` (idempotent `reconnect_matched()` actuator on a serial queue + a 15s `dispatch_source`
...
silently miss the MT2 when the service bit is set: `tools/mt2_linkstated.m:38,60` (this is why `19:23:49`
...
1. **Reliable repro (no reboot):** the cold state must be reproduced by getting the MT2 into Apple-HID BASIC mode first
   (`kextunload MavericksVoodooInputHost` + `mt2_linkstated --bounce-once` → confirm `IOBluetoothHIDDriver`=1), THEN reload our
...
re-enable spam.** Built a reliable no-reboot repro: `kextunload MavericksVoodooInputHost` → `mt2_linkstated --bounce-once`
...
had PSM 19 open). **FIX (proven live in the same session):** a plain `mt2_linkstated --bounce-once`
...
| Transport **orchestration** (pair/handoff) | presence observer + `mt2_linkstated` daemon | ❌ |
