...
# The old update-check agent (and the updater's per-user prefs) belong to shipyard's agent-load snippet (sourced by the postinstall); a
...
# --- idempotent: a second install (e.g. a reinstall of the same version) is harmless ---
run pre "$VOL/"; run post "$VOL/"
[ -f "$VOL/Library/LaunchDaemons/$NEW.plist" ] && pass "a second run is harmless" || bad "a second run removed the new daemon"
