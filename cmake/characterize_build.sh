...
# Userland post-10.9 leaks are covered separately by shipyard's assert_binary_compatible.sh,
# run as a per-target POST_BUILD via mavericks_assert_binary_compatible().
#
...
#   VoodooInputMavericks.kext/Contents/MacOS/VoodooInputMavericks   (+ Contents/Info.plist)
set -eu
...
emit() {   # <build-dir> <out-dir>
  em_bd=$1; em_out=$2
  mkdir -p "$em_out/kext"
  em_kbin="$em_bd/VoodooInputMavericks.kext/Contents/MacOS/VoodooInputMavericks"
  em_kplist="$em_bd/VoodooInputMavericks.kext/Contents/Info.plist"
  ...
  # post-10.9 leaks are covered separately by shipyard's assert_binary_compatible.sh. Keeping this
